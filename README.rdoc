...
* Rails >= 3.1 and Ruby >= 1.9.3 are required. It's time to make the jump to
...
* Edit config/routes.rb, adding this line:
    <tt>mount Perens::InstantUser::Engine => '/'</tt>
...
* Edit one of your environment files, like config/application.rb, to add the
  ...
  system. While testing, this might be:
    <tt>config.action_mailer.default_url_options = { :host => 'localhost:3000' }</tt>
